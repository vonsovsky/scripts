 ...
 * @package    dibi
 * @version    1.01 (revision 224 released on 2009/06/30 16:08:49)
 */
...
new
InvalidArgumentException('Arguments must be array or ArrayObject.');}return$this->command()->insert()->into('%n',$table,'(%n)',array_keys($args))->values('%l',array_values($args));}public
function
...
public
function
setWithTables($val){if($val){$cols=array();foreach($this->getMeta()as$info){$name=$info['fullname'];if(isset($cols[$name])){$fix=1;while(isset($cols[$name.'#'.$fix]))$fix++;$name.='#'.$fix;}$cols[$name]=TRUE;}$this->withTables=array_keys($cols);}else{$this->withTables=FALSE;}return$this;}final
public
function
...
function
asDate($key,$format=NULL){$value=$this[$key];if($value===NULL||$value===FALSE){return$value;}else{$value=is_numeric($value)?(int)$value:strtotime($value);return$format===NULL?$value:date($format,$value);}}public
function
...
as$k=>$v){$pair=explode('%',$k,2);$kx[]=$this->delimite($pair[0]);$vx[]=$this->formatValue($v,isset($pair[1])?$pair[1]:(is_array($v)?'ex':FALSE));}return'('.implode(', ',$kx).') VALUES ('.implode(', ',$vx).')';case'by':foreach($value
...
instanceof
DateTime)return$value=$value->format('U');$this->hasError=TRUE;return'**Unexpected '.gettype($value).'**';}private
function
...
new
InvalidArgumentException('Bad ticket number.');}list($connection,$event,$sql)=$this->tickets[$ticket];if(($event&$this->filter)===0)return;if($event&self::QUERY){try{$count=$res
instanceof
...
VERSION='1.01';const
REVISION='224 released on 2009/06/30 16:08:49';const
